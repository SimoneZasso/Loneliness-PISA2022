...
# Exctract Parameters for Partial Mediation Analysis (i.e., countries with a RMSEA_d > .08)

run_sem_analysis_PART_MED <- function(country_id) {
  
# country_id = 8  # This is to try the function only on one country (e.g., 8)
...
# Do not fit model 2, since RMSEA_d > .08
  ...
  # Extract path a, path b, path c (since Model 1 is inspected), and R^2
  
  param_table <- m1_sem_country[["pe"]] %>% 
    filter(label == "a" | label == "b" | label == "c"| op == "r2") %>% 
    filter(lhs == "Math_ach" & rhs == "Math_ach" | label == "a" | label == "b" |label == "c")
  ...
  param_table2 <- data.frame(
    ...
    path_c = round(param_table[param_table$label == "c", "est"], 3),
    std_err_c = round(param_table[param_table$label == "c", "se"], 3),
    path_c_std = round(param_table[param_table$label == "c", "std.all"], 3),
    path_c_pval = round(param_table[param_table$label == "c", "pvalue"], 3),
    Ach_r_square = round(param_table[param_table$op == "r2", "est"]*100, 2)
  )   #%>%   add_row(., .after = 1)  # Deleted, because we need only Model 1 Results
  # 
  
  # Bind resume table and parameters table
  
   final_table_fit_param <- cbind(resume_table_m1, param_table2)
  
  # Mediation                                   
  
   final_table_med <- med_mcmc(
     a = final_table_fit_param[final_table_fit_param$Model==1, "path_a"],
     b = final_table_fit_param[final_table_fit_param$Model==1, "path_b"],
     se_a = final_table_fit_param[final_table_fit_param$Model==1, "std_err_a"],
     se_b = final_table_fit_param[final_table_fit_param$Model==1, "std_err_b"],
     cov_ab_number = 0,
     D = 0
   )
